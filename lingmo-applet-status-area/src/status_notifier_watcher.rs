...
const DBUS_NAME: &str = "com.lingmoos.LingmoStatusNotifierWatcher";
const OBJECT_PATH: &str = "/LingmoStatusNotifierWatcher";
...
pub async fn cosmic_register(conn: &zbus::Connection) -> zbus::Result<()> {
    let cosmic_watcher = LingmoAppletStatusNotifierWatcherProxy::new(conn).await?;
...
#[zbus::proxy(
    interface = "com.lingmoos.LingmoStatusNotifierWatcher",
    default_service = "com.lingmoos.LingmoStatusNotifierWatcher",
    default_path = "/LingmoStatusNotifierWatcher"
)]
trait LingmoAppletStatusNotifierWatcher {
    async fn register_applet(&self) -> zbus::Result<()>;
}

struct LingmoAppletStatusNotifierWatcher {
    applets: HashSet<zbus::names::UniqueName<'static>>,
    unique_names: UniqueNames,
}

#[zbus::interface(name = "com.lingmoos.LingmoStatusNotifierWatcher")]
impl LingmoAppletStatusNotifierWatcher {
...
pub async fn run_inner() -> zbus::Result<()> {
    ...
    conn.object_server()
        .at(
            OBJECT_PATH,
            LingmoAppletStatusNotifierWatcher {
                applets: HashSet::new(),
    ...
    let interface = conn
        .object_server()
        .interface::<_, LingmoAppletStatusNotifierWatcher>(OBJECT_PATH)
        .await?;
...
async fn refresh_task(
    interface: zbus::object_server::InterfaceRef<LingmoAppletStatusNotifierWatcher>,
    abort_handle: futures::future::AbortHandle,
) {
