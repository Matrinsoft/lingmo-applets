...
use config::{LingmoPanelButtonConfig, IndividualConfig, Override};
...
enum Msg {
    Press,
    ConfigUpdated(LingmoPanelButtonConfig),
...
impl cosmic::Application for Button {
    ...
    type Flags = Desktop;
    const APP_ID: &'static str = "com.lingmoos.LingmoPanelButton";

    fn init(core: cosmic::app::Core, desktop: Desktop) -> (Self, app::Task<Msg>) {
        let config = Config::new(Self::APP_ID, LingmoPanelButtonConfig::VERSION)
            .ok()
            .and_then(|c| LingmoPanelButtonConfig::get_entry(&c).ok())
            .unwrap_or_default()
            .configs
