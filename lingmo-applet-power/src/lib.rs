...
pub mod lingmo_session;
mod localize;
pub mod session_manager;

use crate::{lingmo_session::LingmoSessionProxy, session_manager::SessionManagerProxy};
...
impl cosmic::Application for Power {
    ...
    const APP_ID: &'static str = "com.lingmoos.LingmoAppletPower";
...
async fn log_out() -> zbus::Result<()> {
    ...
    if let Some("pop:GNOME") = session_type.as_ref().map(|s| s.trim()) {
        ...
        let lingmo_session = LingmoSessionProxy::new(&connection).await?;
        lingmo_session.exit().await?;
    }
    Ok(())
}
